...
def process_dir(strModel, strInputDir, strOutputDir, strFramesExt='.jpg', bound=20):
	moduleNetwork = Network(strModel).cuda().eval()
	listFramesPath = sorted(glob(osp.join(strInputDir, '*%s' % strFramesExt)))
	...
	tensorFirst = torch.FloatTensor(first_batch)
	tensorSecond = torch.FloatTensor(second_batch)
	...
	for intIndex in range(tensorOutput.shape[0]):
		...
		# fileOutput = open(osp.join(strOutputDir, '%05d.flo'%(intIndex)), 'wb')
		# numpy.array([80, 73, 69, 72], numpy.uint8).tofile(fileOutput)
		# numpy.array([tensorSave.size(2), tensorSave.size(1)], numpy.int32).tofile(fileOutput)
...
if __name__ == '__main__':
	...
	parser.add_argument('--outputDir', dest='strOutputDir', type=str, help='Path to output dir where flow will be saved')
	args = parser.parse_args()

	# profile_dir_processing(args.strModel, args.strFramesDir, args.strFramesExt, args.strOutputDir)
	process_dir(args.strModel, args.strFramesDir, args.strOutputDir, args.strFramesExt)
