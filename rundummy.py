...
def process_image_pair(strModel, strFirst, strSecond, strOut):
	...
	imageFirst = load_rgb(strFirst)
	imageSecond = load_rgb(strSecond)
	c, h, w = imageFirst.shape

	timeStart = time.time()
	batchSize = 40
	for batchId in range(448):
		batchFirst = numpy.zeros((batchSize, c, h, w), dtype=numpy.float32)
		batchSecond = numpy.zeros((batchSize, c, h, w), dtype=numpy.float32)
		for i in range(batchFirst.shape[0]):
			batchFirst[i, ...] = imageFirst
			batchSecond[i, ...] = imageSecond

		tensorFirst = torch.FloatTensor(batchFirst)
		tensorSecond = torch.FloatTensor(batchSecond)

		tensorOutput = estimate(moduleNetwork, tensorFirst, tensorSecond)

		for i in range(tensorOutput.shape[0]):
			fileId = batchId * batchSize + i
			strOutputPath = os.path.join(strOut, '%d.flo' % fileId)
			fileOutput = open(strOutputPath, 'wb')

			tensorSave = tensorOutput[i, ...]
			...
			numpy.array(tensorSave.numpy().transpose(1, 2, 0), numpy.float32).tofile(fileOutput)

			fileOutput.close()

	timeProcess = time.time() - timeStart
	print('Total time: ', timeProcess)
...
if __name__ == '__main__':
	...
	parser.add_argument('--out', dest='strOut', type=str, default='./output', help='Path to .flo save file')
